...
async def logger_factory(app,hanler):
    async def logger(request):
        logging.info('Request: %s %s' % (request.method,request.path))
        # await asyncio.sleep(0.3)
        return (await hanler(request))
    return logger
    ...
    async def response(request):
        ...
        if isinstance(r,dict):
            ...
            if template is None:
                ...
                resp.content_type = 'application/json;chatset=utf-8'
                return resp
            else:
...
async def init(loop):
    await orm.create_pool(loop=loop, host='127.0.0.1', port=3306, user='root', password='gjj123456', db='awesome')
